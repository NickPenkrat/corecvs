...
void KeyPointDetectionStage::run(FeatureMatchingPipeline *pipeline)
{
    ...
    parallelable_for<size_t, ParallelDetector>(0, N, CORE_MAX(N / 16, (size_t)1)
        , ParallelDetector(pipeline, detectorType));
#endif
...
public:
    ...
    ParallelExtractor(FeatureMatchingPipeline* pipeline, DescriptorType descriptorType)
        : pipeline(pipeline), descriptorType(descriptorType) {}
};
//#endif
...
void DescriptorExtractionStage::run(FeatureMatchingPipeline *pipeline)
{
    ...
    parallelable_for<size_t, ParallelExtractor>(0, N, CORE_MAX(N / 16, (size_t)1), ParallelExtractor(pipeline, descriptorType));
...
MatchingStage::MatchingStage(DescriptorType type, size_t responsesPerPoint)
    : descriptorType(type), responsesPerPoint(responsesPerPoint)
{}
...
public:
    void operator() (const BlockedRange<size_t>& r) const
    {
      //size_t N = pipeline->images.size();
        ...
        for (size_t i = r.begin(); i != r.end(); ++i)
        {
            ...
            FOREACH (auto& v, rawMatches.matches[s])
            {
                FOREACH (auto& m, v)
                {
                    m.featureQ = query.queryFeatures[m.featureQ]; //TODO: modify const object!!!
                    m.featureT = query.trainFeatures[m.featureT]; //TODO: modify const object!!!
                }
            }
    ...
    ParallelMatcher(FeatureMatchingPipeline* pipeline, DescriptorType descriptorType, size_t responsesPerPoint)
        : pipeline(pipeline), descriptorType(descriptorType), responsesPerPoint(responsesPerPoint)
    {}
};
//#endif
...
void MatchingStage::run(FeatureMatchingPipeline *pipeline)
{
    ...
    parallelable_for<size_t, ParallelMatcher>(0, S, CORE_MAX(S / 16, (size_t)1)
...
class ParallelMatcherRefiner
{
    ...
    static const double scaleThreshold;     // = 0.95;  // const double isn't allowed!
           const size_t responsesPerPoint;  // = 2
...
public:
    void operator() (const BlockedRange<size_t>& r) const
    {
        ...
        for (size_t perm_id = r.begin(); perm_id < r.end(); ++perm_id)
        {
            ...
            for (size_t si = 0; si < reqs.size(); ++si)
            {
                ...
                // It's time to replace indicies
                for (std::deque<std::array<RawMatch, 2>>::iterator it = rawMatches.matches[s].begin();
                     it != rawMatches.matches[s].end();
                     ++it)
                {
                    std::array<RawMatch, 2> &v = *it;
                    for (std::array<RawMatch, 2>::iterator m = v.begin(); m != v.end() && m->isValid(); ++m)
    ...
    ParallelMatcherRefiner(FeatureMatchingPipeline* pipeline
        , DescriptorType descriptorType
        , size_t responsesPerPoint
...
const double ParallelMatcherRefiner::scaleThreshold = 0.95;
//#endif

MatchAndRefineStage::MatchAndRefineStage(DescriptorType descriptorType, double scaleThreshold)
    : descriptorType(descriptorType), scaleThreshold(scaleThreshold)
{}
...
void MatchAndRefineStage::run(FeatureMatchingPipeline *pipeline)
{
    ...
    for (size_t I = 0; I < N; ++I)
    {
        for (size_t J = I + 1; J < N; ++J)
        {
            ...
            std::cerr << "Computed all matches for " << I << ":" << J << std::endl;
        }
    }
#else
    //tbb::parallel_for (tbb::blocked_range<size_t>(0, P, std::max(P / 16,(size_t)1)), ParallelMatcherRefiner(pipeline,descriptorType, responsesPerPoint, &first, &next, &idx));

    parallelable_for<size_t, ParallelMatcherRefiner>(0, P, CORE_MAX(P / 16, (size_t)1)
        , ParallelMatcherRefiner(pipeline, descriptorType, responsesPerPoint, &first, &next, &idx));
#endif
