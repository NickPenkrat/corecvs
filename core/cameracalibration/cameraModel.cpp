...
Matrix33 PinholeCameraIntrinsics::getInvKMatrix33() const
{
    ...
    return Matrix33 (
       invF.x() , - skew() * invF.x() * invF.y() ,   ( principalY() * skew() * invF.y() - principalX()) * invF.x(),
          0.0   ,           invF.y()            ,                                       - principalY()  * invF.y(),
          0.0   ,             0.0               ,                              1.0
    );
}
...
{
    return atan((sizeY() / 2.0) / focalY()) * 2.0;
}

double PinholeCameraIntrinsics::getHFov() const
{
    return atan((sizeX() / 2.0) / focalX()) * 2.0;
}
