...
DescriptorExtractor* OpenCvDescriptorExtractorProvider::getDescriptorExtractor(const DescriptorType &type)
{
	...
	SWITCH_TYPE(SIFT,
		...
		SWITCH_TYPE(ORB,
                return new OpenCvDescriptorExtractorWrapper(new cv::ORB(16000, orbParams.scaleFactor, orbParams.nLevels, orbParams.edgeThreshold, orbParams.firstLevel, orbParams.WTA_K, orbParams.scoreType, orbParams.patchSize));)
		return 0;
}
