...
/*  Update UD factorization, for sparse P matrix                         */
/*                                                                       */
/*  Transitionsmatrizen                                                  */
/*                                                                       */
...
/*************************************************************************/
/*
double *diagonalMatrix,       Diagonale von P = U D U~

double* upperTriangleMatrix,      Upper Triangle von P = U D U~

COOSparseMatrix* transitionMatrix,    Transitions-Matrix

double *errorCovMatrix,        Systemfehler-Kovarianz-Matrix
                          System error if covariance matrix
...
*/
void KalmanWorkspace::calculatePrediction(UDUTDecomposed *P, Matrix *Q, COOSparseMatrix *F)
{
    /* TODO: Make a deep check of the passed parameter */
    kfudt(P->diagonal->elements, P->upper->elements, F, &(Q->a(0,0)), P->getSize(), this);
}
 ...
 */
void KalmanWorkspace::calculateInnovation(
        Matrix         *R,
        UDUTDecomposed *P,
        Matrix         *H,
        DyVector       *y,
        ...
        Matrix         *stateVectorInnovation)
{
    ...
    for(int iy = 0; iy < y->number; iy++)
    {
        ...
        {
            corrector->a(ix) += correctorStep->a(ix);
            if(iy < y->number - 1 )
            {
                int nextIndex = y->index[iy+1];
                y->dy[nextIndex] -= H->a(nextIndex, ix) * corrector->a(ix);
            }
        }
    }
    ...
    for(int ix = 0; ix < stateVector->w; ix++)
    {
        stateVectorInnovation->a(ix) = stateVector->a(ix) + corrector->a(ix);
    }
      ...
      )
{
    ...
    //Matrix *workVector = new Matrix(1, stateVector->w, false);

    for (t = 0; t < stepsToMake; t++) {

        //workVector->fillWith(stateVector);
        ...
        for (i = 0; i < j; i++) {
            ...
            stateVectorPredition->a(ir) += F->elements[i].value * stateVector->a(ic);
        }
        ...
        if (B != NULL) {
            ...
            for (i = 0; i < j; i++) {
                ...
                stateVectorPredition->a(ir) += B->elements[i].value * u->a(ic);
            }
        }
    }
