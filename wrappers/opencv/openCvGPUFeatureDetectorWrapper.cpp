...
OpenCvGPUFeatureDetectorWrapper::OpenCvGPUFeatureDetectorWrapper(
    ...
    : detectorSURF_CUDA(detector), holder(0) {}

OpenCvGPUFeatureDetectorWrapper::OpenCvGPUFeatureDetectorWrapper(
    SmartPtrHolder *holder)
    : holder(holder), detectorSURF_CUDA(0) {}
...
double
OpenCvGPUFeatureDetectorWrapper::getProperty(const std::string &name) const {
  CORE_UNUSED(name);
  return 0.0;
}

void OpenCvGPUFeatureDetectorWrapper::setProperty(const std::string &name,
                                                  const double &value) {
  CORE_UNUSED(name);
  CORE_UNUSED(value);
}
...
void OpenCvGPUFeatureDetectorWrapper::detectImpl(
    corecvs::RuntimeTypeBuffer &image, std::vector<KeyPoint> &keyPoints, int K,
    void *pRemapCache) {
  ...
  if (detectorSURF_CUDA) {
    cv::cuda::GpuMat img(OpenCVTools::convert(image));
    if (pRemapCache) {
      GpuMat remapped;
    ...
    {
      cv::UMat img(OpenCVTools::convert(image));
      if (pRemapCache) {
        UMat remapped;
  ...
  if (detectorSURF_CUDA || detectorORB_CUDA) {
    cv::gpu::GpuMat img(OpenCVTools::convert(image));
    if (pRemapCache) {
      GpuMat remapped;
      CudaRemapCache *p = (CudaRemapCache *)(pRemapCache);
    ...
    cv::ocl::oclMat img(OpenCVTools::convert(image));
    if (pRemapCache) {
      oclMat remapped;
      OpenCLRemapCache *p = (OpenCLRemapCache *)(pRemapCache);
      cv::ocl::remap(img, remapped, p->mat0, p->mat1, cv::INTER_NEAREST,
                     cv::BORDER_CONSTANT);
      img = remapped;
    }
  ...
  keyPoints.clear();
  FOREACH(const cv::KeyPoint &kp, kps) { keyPoints.push_back(convert(kp)); }

  std::sort(keyPoints.begin(), keyPoints.end(),
            [](const KeyPoint &l, const KeyPoint &r) {
              return l.response > r.response;
            });
...
bool FindGPUDevice(bool &cudaApi) {
  ...
  if (!initProvider) // if no cuda device found, try use first found opencl gpus
  {
    ...
    // it crashed!!! getOpenCLDevices(oclDevices,
    ...
    if (oclDevices.size()) {
      for (uint i = 0; i < oclDevices.size(); i++) {
        ...
        if (device->deviceVersionMajor > 1 ||
            device->deviceVersionMinor >= 2) // check opencl device support 1.2
        {
...
void init_opencv_gpu_detectors_provider() {
  ...
  if (FindGPUDevice(cudaApi))
    FeatureDetectorProvider::getInstance().add(
        new OpenCvGPUFeatureDetectorProvider(cudaApi));
}

OpenCvGPUFeatureDetectorProvider::OpenCvGPUFeatureDetectorProvider(
    bool _cudaApi)
    : cudaApi(_cudaApi) {}
...
FeatureDetector *OpenCvGPUFeatureDetectorProvider::getFeatureDetector(
    const DetectorType &type, const std::string &params) {
  ...
  if (type == "SURF_GPU") {
    if (cudaApi) {
      delete holder;
      holder = 0;
      return new OpenCvGPUFeatureDetectorWrapper(new cv::cuda::SURF_CUDA(
          surfParams.hessianThreshold, surfParams.octaves,
          surfParams.octaveLayers, surfParams.extended, 0.01f,
          surfParams.upright));
    } else {
      cv::Ptr<cv::xfeatures2d::SURF> ptr = cv::xfeatures2d::SURF::create(
          surfParams.hessianThreshold, surfParams.octaves,
          surfParams.octaveLayers, surfParams.extended, surfParams.upright);
      holder->set(ptr);
  ...
  if (type == "ORB_GPU") {
    if (cudaApi) {
      cv::Ptr<cv::cuda::ORB> ptr = cv::cuda::ORB::create(
          orbParams.maxFeatures, orbParams.scaleFactor, orbParams.nLevels,
          orbParams.edgeThreshold, orbParams.firstLevel, orbParams.WTA_K,
  ...
  if (cudaApi) {
    SWITCH_TYPE(
        SURF_GPU,
        return new OpenCvGPUFeatureDetectorWrapper(new cv::gpu::SURF_GPU(
            surfParams.hessianThreshold, surfParams.octaves,
            surfParams.octaveLayers, surfParams.extended, 0.01f,
            surfParams.upright));)
    SWITCH_TYPE(
        ORB_GPU,
        return new OpenCvGPUFeatureDetectorWrapper(new cv::gpu::ORB_GPU(
            orbParams.maxFeatures, orbParams.scaleFactor, orbParams.nLevels,
            orbParams.edgeThreshold, orbParams.firstLevel, orbParams.WTA_K,
            orbParams.scoreType, orbParams.patchSize));)
  } else
    SWITCH_TYPE(
        SURF_GPU,
        return new OpenCvGPUFeatureDetectorWrapper(new cv::ocl::SURF_OCL(
            surfParams.hessianThreshold, surfParams.octaves,
            surfParams.octaveLayers, surfParams.extended, 0.01f,
            surfParams.upright));)
