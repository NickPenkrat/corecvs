...
int OpenCVMovingObjectFlow::requestResultsi(int parameterValue)
{
    return 0;
}
...
void MeshFlow::init( const int image_height,
    ...
    mesh_row_ = mesh_row;
    mesh_col_ = mesh_col;
    mesh_width_ =  ( ( float ) image_width_ / ( float ) mesh_col_ );
    mesh_height_ =  ( ( float ) image_height_ / ( float ) mesh_row_ );
    mesh_ransac_row_ = mesh_ransac_row;
    mesh_ransac_col_ = mesh_ransac_col;
    mesh_ransac_height_ = ( ( float ) image_height_ / ( float ) mesh_ransac_row_ );
    mesh_ransac_width_ =  ( ( float ) image_width_ / ( float ) mesh_ransac_col_ );
    median_filter_size_x_ = median_filter_size_x;
    median_filter_size_y_ = median_filter_size_y;
    mesh_vertex_x_ = Mat::zeros ( mesh_row_ + 1, mesh_col_ + 1, DataType<float>::type );
    mesh_vertex_y_ = Mat::zeros ( mesh_row_ + 1, mesh_col_ + 1, DataType<float>::type );
...
void MeshFlow::ResetInternalMemory() {
    mesh_flow_global_x_ = Mat::zeros ( mesh_row_ + 1, mesh_col_ + 1, DataType<float>::type );
    mesh_flow_global_y_ = Mat::zeros ( mesh_row_ + 1, mesh_col_ + 1, DataType<float>::type );
    mesh_flow_local_x_ = Mat::zeros ( mesh_row_ + 1, mesh_col_ + 1, DataType<float>::type );
    mesh_flow_local_y_ = Mat::zeros ( mesh_row_ + 1, mesh_col_ + 1, DataType<float>::type );
    feat_prev_.clear();
    feat_cur_.clear();
    feat_status_.clear();
    ...
    for ( int i = 0; i < mesh_ransac_col_ * mesh_ransac_row_; ++i ) {
...
unsigned char MeshFlow::GetGrayValue ( Vec3b pix ) {
    float pix_gray;
    float wegith_blue = ( float ) 0.1140;
    float weight_green = ( float ) 0.5870;
    float weight_red = ( float ) 0.2989;
...
void MeshFlow::MeshGridRANSACFitting ( const vector<Point2f> feat,
    ...
    for ( int i = 0; i < mesh_ransac_row_ * mesh_ransac_col_; ++i ) {
        ...
        float t_mesh_size_r = ( float ) image_height_ / ( float ) mesh_ransac_row_;
        float t_mesh_size_c = ( float ) image_width_ / ( float ) mesh_ransac_col_;

        for ( int j = 0; j < ( int ) feat.size(); ++j ) {
            int r, c;
            ComputeMeshIndex ( feat.at ( j ).y, feat.at ( j ).x, t_mesh_size_r, t_mesh_size_c, ( float ) mesh_ransac_row_, ( float ) mesh_ransac_col_, r, c );

            if ( r * mesh_ransac_col_ + c == i ) {
    ...
    if ( type == REGIONWISE ) {
        for ( int i = 0; i < mesh_ransac_row_ * mesh_ransac_col_; ++i ) {
            ...
            for ( int j = 0; j < num_feat; ++j ) {
                ComputeMeshIndex ( feat1.at ( j ).y, feat1.at ( j ).x, t_mesh_size_r, t_mesh_size_c, ( float ) mesh_ransac_row_, ( float ) mesh_ransac_col_, r, c );
...
void MeshFlow::InitializeGlobalMeshFlow ( const Mat H ) {
    ...
    for ( int i = 0; i < mesh_row_ + 1; ++i ) {
        for ( int j = 0; j < mesh_col_ + 1; ++j ) {
            Point2f vertex;
    ...
    for ( int i = 0; i < mesh_row_ + 1; ++i ) {
        for ( int j = 0; j < mesh_col_ + 1; ++j ) {
            int vertex_idx = i * ( mesh_col_ + 1 ) + j;
...
void MeshFlow::SuppressingMotionNoise ( const vector<Point2f> refined_feat_vec,
    ...
    Mat t_local_mesh_flow_x = Mat::zeros ( mesh_row_ + 1, mesh_col_ + 1, CV_32FC ( MAX_CHANNEL_SIZE ) );
    Mat t_local_mesh_flow_y = Mat::zeros ( mesh_row_ + 1, mesh_col_ + 1, CV_32FC ( MAX_CHANNEL_SIZE ) );
    Mat t_local_mesh_store_idx_x = Mat::zeros ( mesh_row_ + 1, mesh_col_ + 1, DataType<int>::type );
    Mat t_local_mesh_store_idx_y = Mat::zeros ( mesh_row_ + 1, mesh_col_ + 1, DataType<int>::type );
    float t_mesh_size_r = ( float ) image_height_ / ( float ) mesh_row_;
    float t_mesh_size_c = ( float ) image_width_ / ( float ) mesh_col_;
...
Mat MeshFlow::getFeatureMatchingResult () {
    ...
    for ( int p = 0; p < feat_prev_.size(); ++p ) {
