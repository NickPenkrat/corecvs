...
// this name is needed to retain compatibility with focusEstimator
#define CRGB24BUFFER_H_
/**
* \file rgbBufferT.h
* \brief RGBTBuffer template class definition
*
* \date Oct 15, 2015
* \author alexander
* \author pvasilev
        ...
        */
            ...
            FunctionArgs &f)
        {
            ...
            for (i = 1; i < h - 1; i++)
            {
                for (j = 1; j < w - 1; j++)
                {
                    ...
                    if (r < ls || r > hs ||
                        l < ls || l > hs)
                        element(i, j) = RGBTColor<T>::gray(128);
                }
            }
        ...
        void fillWithYUYV(T *yuyv)
        {
            for (int i = 0; i < h; i++)
            {
                ...
                for (; j + 2 <= w; j += 2)
                {
                    ...
                    element(i, j) = RGBTColor<T>(r1, g1, b1);
                    element(i, j + 1) = RGBTColor<T>(r2, g2, b2);
                    yuyv += 4;
                }
            }
        }
        ...
        void dropValueAndSatuation(void)
        {
            for (int i = 0; i < h; i++)
            {
                for (int j = 0; j < w; j++)
                {
                    element(i, j) = RGBTColor<T>::FromHSV(element(i, j).hue(), 255, 255);
                }
            }
        }
        ...
        void dropValue()
        {
            for (int i = 0; i < h; i++)
            {
                for (int j = 0; j < w; j++)
                {
                    element(i, j) = RGBTColor<T>::FromHSV(element(i, j).hue(), element(i, j).saturation(), 255);
                }
            }
        }
