...
struct DetectorHolder
{
    ...
    cv::Feature2D *get() {
        switch (tag) {
        case SIFT:
            return holder.sift.get();
        case SURF:
            return holder.surf.get();
        case STAR:
            return holder.star.get();
        case FAST:
            return holder.fast.get();
        case BRISK:
            return holder.brisk.get();
        case ORB:
            return holder.orb.get();
        case AKAZE:
            return holder.akaze.get();
        default:
            return NULL;
        }
    }
