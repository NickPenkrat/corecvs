 ...
 *
 **/
void DistPointsFunction::operator ()(const double in[], double out[])
{
    int count = 0;
    ...
    for (unsigned i = 0; i < mSample.size(); i++)
    {
        ...
        for (unsigned j = 0; j < straight.size(); j ++)
        {
            ...
            ell.addPoint(newStraight[j]);
        }

        Vector2dd mean = ell.getCenter();
        Vector2dd normal = ell.getMainDirection().leftNormal();

        for (unsigned j = 0; j < straight.size(); j++)
        {
            out[count] = (newStraight[j] - mean) & normal;
            count++;
        }
    }
 ...
 **/
Matrix DistPointsFunction::getJacobian(const double in[], double delta)
{
    CORE_UNUSED(delta);
    if (mSimpleJacobian) {
 ...
 **/
int DistPointsFunction::getOutputs(const vector<vector<Vector2dd> > &sample)
{
    int pointsNum = 0;
