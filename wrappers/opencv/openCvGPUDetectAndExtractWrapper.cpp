...
void OpenCvGPUDetectAndExtractWrapper::detectAndExtractImpl(
    corecvs::RuntimeTypeBuffer &image, std::vector<::KeyPoint> &keyPoints,
    corecvs::RuntimeTypeBuffer &descriptors, int nMaxKeypoints,
    void *pRemapCache) {
  ...
  if (detectorSURF_CUDA || detectorORB_CUDA) {
    GpuMat img(OpenCVTools::convert(image));
    if (pRemapCache) {
      GpuMat remapped;
    ...
    if (detectorSURF_CUDA) {
      ...
      // detectorSURF_CUDA->keypointsRatio = ( float )nMaxKeypoints /
      // img.size().area();
    ...
    oclMat img(OpenCVTools::convert(image));
    if (pRemapCache) {
      oclMat remapped;
    ...
    // max keypoints = min(keypointsRatio * img.size().area(), 65535)
    // detectorSURF_OCL->keypointsRatio = ( float )nMaxKeypoints /
    // img.size().area();
    ...
    Mat img(OpenCVTools::convert(image));
  ...
  for (int i = 0; i < newCount; i++) {
    ...
    //	memcpy(sortedDescriptors.ptr(i, j), cv_descriptors.ptr(idx, j),
    //szElement);
...
void OpenCvGPUDetectAndExtractWrapper::setProperty(const std::string &name,
                                                   const double &value) {
  CORE_UNUSED(name);
  CORE_UNUSED(value);
}
...
double
OpenCvGPUDetectAndExtractWrapper::getProperty(const std::string &name) const {
  CORE_UNUSED(name);
  return 0;
}

OpenCvGPUDetectAndExtractWrapper::OpenCvGPUDetectAndExtractWrapper(
    cv::ORB *detectorORB, cv::SURF *detectorSURF,
    cv::gpu::SURF_GPU *detectorSURF_CUDA, cv::gpu::ORB_GPU *detectorORB_CUDA,
    cv::ocl::SURF_OCL *detectorSURF_OCL)
    : detectorORB(detectorORB), detectorSURF(detectorSURF),
      detectorSURF_CUDA(detectorSURF_CUDA), detectorORB_CUDA(detectorORB_CUDA),
...
bool init_opencv_gpu_detect_and_extract_provider(bool &cudaApi) {
  ...
  if (FindGPUDevice(cudaApi)) {
    DetectAndExtractProvider::getInstance().add(
        new OpenCvGPUDetectAndExtractProvider(cudaApi));
    return true;
  }
...
OpenCvGPUDetectAndExtractProvider::OpenCvGPUDetectAndExtractProvider(
    bool cudaApi)
    : cudaApi(cudaApi) {}
...
DetectAndExtract *OpenCvGPUDetectAndExtractProvider::getDetector(
  ...
  if (cudaApi) {
    if (detectorType == "SURF" && descriptorType == "SURF")
      return new OpenCvGPUDetectAndExtractWrapper(
          0, new cv::SURF(surfParams.hessianThreshold, surfParams.octaves,
                          surfParams.octaveLayers, surfParams.extended,
                          surfParams.upright));

    if (detectorType == "ORB" && descriptorType == "ORB")
      return new OpenCvGPUDetectAndExtractWrapper(new cv::ORB(
...
bool OpenCvGPUDetectAndExtractProvider::provides(
    const DetectorType &detectorType, const DescriptorType &descriptorType) {
