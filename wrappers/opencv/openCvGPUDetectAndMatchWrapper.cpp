...
void OpenCvGPUDetectExtractAndMatchWrapper::detectExtractAndMatchImpl(
    FeatureMatchingPipeline &pipeline, int nMaxKeypoints,
    int numResponcesPerPoint) {
  ...
  for (size_t i = 0; i < numImages; i++) {
    ...
    std::unique_ptr<BufferReader> reader(
        BufferReaderProvider::getInstance().getBufferReader(image.filename));
    corecvs::RuntimeTypeBuffer img = reader->read(image.filename);

    if (detectorSURF_OCL) {
      GPUImageDetectData gpuImage(cv::ocl::oclMat(OpenCVTools::convert(img)));
      gpuImages.push_back(gpuImage);
    } else {
      GPUImageDetectData gpuImage(cv::gpu::GpuMat(OpenCVTools::convert(img)));
      gpuImages.push_back(gpuImage);
    }
  }
  ...
  for (size_t i = 0; i < numImages; i++) {
    if (detectorSURF_OCL) {
      ...
      detectorSURF_OCL->keypointsRatio =
          (float)nMaxKeypoints / gpuImages[i].oclImg.size().area();
      (*detectorSURF_OCL)(gpuImages[i].oclImg, cv::ocl::oclMat(),
                          gpuImages[i].oclKeypoints,
                          gpuImages[i].oclDescriptors, false);
    } else if (detectorSURF_CUDA) {
      // max keypoints = min(keypointsRatio * img.size().area(), 65535)
      detectorSURF_CUDA->keypointsRatio =
          (float)nMaxKeypoints / gpuImages[i].cudaImg.size().area();
      (*detectorSURF_CUDA)(gpuImages[i].cudaImg, cv::gpu::GpuMat(),
                           gpuImages[i].cudaKeypoints,
                           gpuImages[i].cudaDescriptors, false);
    } else if (detectorORB_CUDA) {
      (*detectorORB_CUDA)(gpuImages[i].cudaImg, cv::gpu::GpuMat(),
                          gpuImages[i].cudaKeypoints,
                          gpuImages[i].cudaDescriptors);
    }
  }
  ...
  for (size_t i = 0; i < numImages; i++) {
    ...
    for (size_t j = 0; j < numImages; j++) {
      ...
      if (i != j) {
        if (matcherBF_CUDA)
          matcherBF_CUDA->knnMatch(gpuImages[i].cudaDescriptors,
                                   gpuImages[j].cudaDescriptors, matches_cv,
                                   numResponcesPerPoint);
        else if (matcherBF_OCL)
          matcherBF_OCL->knnMatch(gpuImages[i].oclDescriptors,
                                  gpuImages[j].oclDescriptors, matches_cv,
                                  numResponcesPerPoint);
      }
  ...
  for (size_t i = 0; i < numImages; i++) {
    ...
    if (detectorSURF_CUDA) {
      detectorSURF_CUDA->downloadKeypoints(gpuImages[i].cudaKeypoints,
                                           keypoints);    // copy GPU -> CPU
      ...
      detectorSURF_OCL->downloadKeypoints(gpuImages[i].oclKeypoints,
                                          keypoints);    // copy GPU -> CPU
    ...
    image.descriptors.mat = OpenCVTools::convert(descriptors);
  }
  ...
  for (size_t i = 0; i < S; i++) {
    ...
    std::vector<std::vector<cv::DMatch>> &matches_cv =
        matchesPerImage[I * numImages + J];
    matches.clear();
    matches.resize(matches_cv.size());

    for (size_t idx = 0; idx < matches.size(); ++idx) {
      for (std::vector<cv::DMatch>::iterator m = matches_cv[idx].begin();
           m != matches_cv[idx].end(); ++m) {
        matches[idx].push_back(convert(*m));
      }
    }

    for (std::vector<std::vector<RawMatch>>::iterator v = matches.begin();
         v != matches.end(); ++v) {
    ...
    for (std::deque<std::array<RawMatch, 2>>::iterator v =
             rawMatches.matches[s].begin();
         v != rawMatches.matches[s].end(); ++v) {
      for (std::array<RawMatch, 2>::iterator m = v->begin();
           m != v->end() && m->isValid(); ++m) {
...
OpenCvGPUDetectExtractAndMatchWrapper::OpenCvGPUDetectExtractAndMatchWrapper(
    SURF_GPU *detector, cv::gpu::BruteForceMatcher_GPU_base *matcher)
    : detectorSURF_CUDA(detector), detectorORB_CUDA(0), detectorSURF_OCL(0),
      matcherBF_CUDA(matcher), matcherBF_OCL(0) {}

OpenCvGPUDetectExtractAndMatchWrapper::OpenCvGPUDetectExtractAndMatchWrapper(
    ORB_GPU *detector, cv::gpu::BruteForceMatcher_GPU_base *matcher)
    : detectorORB_CUDA(detector), detectorSURF_CUDA(0), detectorSURF_OCL(0),
      matcherBF_CUDA(matcher), matcherBF_OCL(0) {}

OpenCvGPUDetectExtractAndMatchWrapper::OpenCvGPUDetectExtractAndMatchWrapper(
    SURF_OCL *detector, cv::ocl::BruteForceMatcher_OCL_base *matcher)
    : detectorSURF_OCL(detector), detectorSURF_CUDA(0), detectorORB_CUDA(0),
      matcherBF_CUDA(0), matcherBF_OCL(matcher) {}
...
bool init_opencv_gpu_detect_extract_and_match_provider(bool &cudaApi) {
  ...
  if (FindGPUDevice(cudaApi)) {
    DetectExtractAndMatchProvider::getInstance().add(
        new OpenCvGPUDetectExtractAndMatchProvider(cudaApi));
    return true;
  }
...
OpenCvGPUDetectExtractAndMatchProvider::OpenCvGPUDetectExtractAndMatchProvider(
    bool _cudaApi)
    : cudaApi(_cudaApi) {}
...
DetectExtractAndMatch *OpenCvGPUDetectExtractAndMatchProvider::getDetector(
    const DetectorType &detectorType, const DescriptorType &descriptorType,
    const MatcherType &matcherType, const std::string &params) {
  ...
  if (cudaApi) {
    if (detectorType == "SURF_GPU" && descriptorType == "SURF_GPU" &&
      ...
      return new OpenCvGPUDetectExtractAndMatchWrapper(
          new cv::gpu::SURF_GPU(surfParams.hessianThreshold, surfParams.octaves,
                                surfParams.octaveLayers, surfParams.extended,
                                0.01f, surfParams.upright),
          new cv::gpu::BruteForceMatcher_GPU_base(
              cv::gpu::BruteForceMatcher_GPU_base::L2Dist));

    if (detectorType == "ORB_GPU" && descriptorType == "ORB_GPU" &&
        matcherType == "BF_GPU") {
      OrbParams orbParams(params);
      return new OpenCvGPUDetectExtractAndMatchWrapper(
          new cv::gpu::ORB_GPU(orbParams.maxFeatures, orbParams.scaleFactor,
                               orbParams.nLevels, orbParams.edgeThreshold,
                               orbParams.firstLevel, orbParams.WTA_K,
                               orbParams.scoreType, orbParams.patchSize),
          new cv::gpu::BruteForceMatcher_GPU_base(
             ...
             matcherType == "BF_GPU")
    return new OpenCvGPUDetectExtractAndMatchWrapper(
        new cv::ocl::SURF_OCL(surfParams.hessianThreshold, surfParams.octaves,
                              surfParams.octaveLayers, surfParams.extended,
                              0.01f, surfParams.upright),
        new cv::ocl::BruteForceMatcher_OCL_base(
            cv::ocl::BruteForceMatcher_OCL_base::L2Dist));
  return 0;
}

bool OpenCvGPUDetectExtractAndMatchProvider::provides(
    const DetectorType &detectorType, const DescriptorType &descriptorType,
